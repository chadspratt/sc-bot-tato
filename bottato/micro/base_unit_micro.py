...
class BaseUnitMicro(GeometryMixin):
    ...
    def _kite(self, unit: Unit, targets: Units, bonus_distance: float = 0, force_move: bool = False) -> UnitMicroType:
        ...
        should_run: bool = (workers_to_avoid.amount > 0 or threats_to_avoid.amount > 0) 
        target: Unit | None = self._get_attack_target(unit, targets, bonus_distance, require_in_range_target=should_run)
        ...
        if workers_to_avoid:
            ...
            worker_center = Point2(cy_center(workers_to_avoid))
        ...
        if target:
            ...
            unit.move(Point2(cy_towards(target.position, unit.position, desired_distance)))
            return UnitMicroType.MOVE
        return UnitMicroType.NONE
    ...
    def _attack(self, unit: Unit, target: Unit) -> bool:
        if target.type_id == UnitTypeId.INTERCEPTOR:
            ...
            unit.move(self.tactics.map.get_pathable_position(self.tactics.enemy.predicted_positions[target.tag], unit))
        else:
            unit.attack(target)
        return True
    ...
    def _retreat_to_better_unit(self, unit: Unit, can_attack: bool) -> bool:
        ...
        closest_threat_distance_sq = self.tactics.enemy.safe_distance_squared(closest_threat, unit)
        ...
        if ally_to_enemy_distance_sq < 900 and ally_to_enemy_distance_sq > self.tactics.enemy.get_attack_range_with_buffer_squared(nearest_unit_to_retreat_to, closest_threat, 0):
            ...
            return True
        elif not can_attack and ally_to_enemy_distance_sq < self.tactics.enemy.safe_distance_squared(unit, closest_threat) * 0.3:
