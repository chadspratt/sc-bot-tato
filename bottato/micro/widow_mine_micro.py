...
class WidowMineMicro(BaseUnitMicro, GeometryMixin):
    ...
    async def _use_ability(self, unit: Unit, target: Point2, health_threshold: float, force_move: bool = False) -> bool:
        ...
        can_attack = unit.weapon_cooldown < self.time_in_frames_to_transform
        if is_burrowed:
            if can_attack:
                if closest_distance <= self.attack_range + 2:
                    return False  # keep burrowed to attack
                sieged_tanks = self.bot.units.of_type(UnitTypeId.SIEGETANKSIEGED)
                closest_tank_to_enemy = self.closest_unit_to_unit(closest_enemy, sieged_tanks) if closest_enemy and sieged_tanks else None
                if closest_tank_to_enemy is None or closest_tank_to_enemy.distance_to_squared(unit) > 36:
                    ...
                    self.unburrow(unit)
                    return True
            elif not self.drilling_claws_researched:
                self.unburrow(unit)
                return True
            return False
        else:
            if closest_distance <= self.attack_range + 3:
                # burrow to attack
                self.burrow(unit)
                return True
            elif closest_distance <= self.attack_range + 6 and can_attack and closest_enemy:
                unit.move(closest_enemy)
                return True
            sieged_tanks = self.bot.units.of_type(UnitTypeId.SIEGETANKSIEGED)
            if closest_enemy and sieged_tanks:
                closest_tank_to_enemy = self.closest_unit_to_unit(closest_enemy, sieged_tanks)
                if closest_tank_to_enemy:
                    burrow_position = closest_tank_to_enemy.position.towards(closest_enemy.position, closest_tank_to_enemy.radius + 1)
