...
def cy_get_turn_speed(unit: Unit, unit_type_id: UnitTypeId) -> float:
    """Get turn speed of unit in radians

    Example:
    ```py
    from cython_extensions import cy_get_turn_speed

    unit: Unit = self.workers[0]
    unit_type_id: UnitTypeId = self.townhalls[0].type_id
    turn_speed: float = cy_get_turn_speed(unit, unit_type_id)
    ```

    Args:
        unit: The actual unit we are checking.
        unit_type_id: The unit type ID integer value.
        angle_error: Some leeway when deciding if a unit is facing the other unit.
        Defaults to 0.3.

    Returns:
        turn speed of unit in radians.
...
def cy_find_aoe_position(
    effect_radius: float, targets: Union[Units, list[Unit]], bonus_tags: set[int] | None = None
) -> Optional[np.ndarray]:
